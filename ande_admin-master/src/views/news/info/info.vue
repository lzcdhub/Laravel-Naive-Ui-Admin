<template>
  <div>
    <n-card :bordered="false" class="mt-2 proCard">
      ...
      >
        <template #tableTitle>
          <n-button type="primary" @click="addFun"> 新增新闻</n-button>
        </template>
      </BasicTable>
    </n-card>
...
<script lang="ts" setup>
  ...
  import { news, newsDel, newsState } from '@/api/news/info';
  import { BasicTable, TableAction } from '@/components/Table';
  import { NSwitch, useDialog } from 'naive-ui';
  import create from '@/views/news/info/create.vue';
  ...
  function handleSwitch(row) {
    ...
    newsState(row.id, { status_f: row.status_f });
  }

  const loadDataTable = async (res) => {
    return await news({ ...res, ...params });
  };
  ...
  function handleDelete(record) {
    dialog.warning({
      ...
      negativeText: '不确定',
      onPositiveClick: () => {
        newsDel(record.id).then(function () {
          reloadTable();
        });
      },
    });
