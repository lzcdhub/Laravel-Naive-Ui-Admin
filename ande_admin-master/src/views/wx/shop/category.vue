...
<script lang="ts" setup>
  ...
  const loadDataTable = async (res: any) => {
    ...
    return await categoryTree(_params);
  };
  ...
  const actionColumn = reactive({
    ...
    render(record) {
      return h(TableAction, {
        ...
        select: (key) => {
          if (key == 'up' || key == 'down') {
            tabbingOrderApi({ operate: key }, record.id).then(function () {
              reloadTable();
            });
          } else if (key == 'add') {
  ...
  function handleDelete(record: Recordable) {
    dialog.warning({
      ...
      negativeText: '不确定',
      onPositiveClick: () => {
        deleteCategory(record.id).then(function () {
          reloadTable();
        });
      },
    });
  ...
  function formSubmit() {
    formRef.value?.validate((errors) => {
      if (!errors) {
        if (formData.id) {
          editCategoryApi(formData, formData.id).then(function () {
            reloadTable();
          });
        } else {
          addCategoryApi(formData).then(function () {
            reloadTable();
          });
        }
      }
